"""
VLM Perception Module: Virtual CCTV Camera Frames -> Vision-Language Model -> Structured Observations

  POST /api/vlm/observe {camera_id, image_b64}  →  VlmObservation {event, zone, severity, blocked, confidence, bbox?}

- Online: Vision-Language Model inference with structured JSON schema.
- Offline: simulated_observation() generates ground-truth observation tagged as raw="simulated".
- Telemetry integration: updates cameras[id].last_observation and emits VLM_OBSERVATION event.
  Autonomous zone lockdown driven by TWIN_VLM_ACTS environment variable (disabled by default).
"""
...
def _client():
    ...
    try:
        ...
        return OpenAI(api_key=key, base_url=base_url or None, timeout=30)
    except Exception:
        return None
...
def simulated_observation(camera_id: str, S: dict[str, Any], layout: dict[str, Any]) -> dict[str, Any]:
    """Generate simulated vision observation from ground truth when VLM is offline."""
...
def observe(camera_id: str, image_b64: str | None, S: dict[str, Any], layout: dict[str, Any]) -> dict[str, Any]:
    ...
    hint.pop("people_in_range", None)  # Mask ground truth before model inference
    result = None
    if image_b64:
        try:
