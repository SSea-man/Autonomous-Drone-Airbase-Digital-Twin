"""
WareTwin Server Engine - Real-time Digital Twin Backend
...
class TwinServer:
    """Holds simulation engine, tick loop, WebSocket connections, and differential state."""
    ...
    def __init__(self, seed: int = 42, db_path: str = "twin.db") -> None:
        ...
        self.last_sent_tick = 0          # Tick count of previous broadcast (base_tick for patches)
        self.last_progress = time.monotonic()   # Monotonic timestamp of last progress (health probe)
        self.loop_errors = 0
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                raise
            except Exception as e:  # Ensure loop resilience against temporary audit write errors
    ...
    async def after_ticks(self) -> None:
        ...
        except Exception as e:  # Audit database failure does not interrupt live telemetry
        ...
        # base_tick tracks previous broadcast tick to prevent client desynchronization
        ...
        if tick % HEATMAP_EVERY == 0:
            for fl in eng.traffic:   # Per-floor traffic heatmap
    ...
    def _robot_patch(self) -> dict[str, Any]:
        """Compute incremental robot telemetry patch against previously broadcast frame."""
        out: dict[str, Any] = {}
    ...
    async def handle(self, ws: WebSocket, raw: str) -> None:
        ...
        if not ok:
            # Extract request_id if present to associate errors with pending client operations
            rid = None
            try:
        ...
        if t == "RESYNC":
            ...
            # Execute LLM query in worker thread to prevent blocking simulation tick
    ...
    async def run_whatif_safe(self, req: dict[str, Any]) -> dict[str, Any]:
        """Clone live engine on main thread and dispatch what-if execution to worker thread."""
...
async def lifespan(app: FastAPI):
    ...
    if server._task and not server._task.done():
        server._task.cancel()
    server.last_progress = time.monotonic()   # Initial startup progress marker
...
# CORS configuration: allow origins from TWIN_CORS_ORIGINS env var or wildcard in dev
...
class GuardMiddleware:
    """ASGI security middleware: enforces Origin verification and request body payload limits."""
    ...
    async def __call__(self, scope: dict[str, Any], receive: Any, send: Any) -> None:
        ...
        async def capped_receive() -> dict[str, Any]:
            ...
            if msg["type"] == "http.request":
                ...
                if total > MAX_BODY_BYTES:
                    tripped = True
                    # Terminate reading and return HTTP 413 Payload Too Large
        ...
        async def guarded_send(msg: dict[str, Any]) -> None:
            ...
            if tripped:
                ...
                return   # Discard downstream response
            await send(msg)
...
async def ws_endpoint(ws: WebSocket) -> None:
    ...
    try:
        ...
        for fl in server.engine.traffic:   # Per-floor traffic heatmap
...
async def health() -> Any:
    """Health check: returns 503 if simulation loop is stalled beyond STALL_S seconds."""
    h = health_payload()
    if not h["ok"]:
...
async def validate_state() -> dict[str, Any]:
    """Validate current state against Pydantic schema."""
...
async def post_vlm(body: VlmObserveBody, request: Request) -> dict[str, Any]:
    """Process live camera frame data URL and return structured VlmObservation."""
    throttle(request, "ai")
...
async def post_whatif(body: dict[str, Any], request: Request) -> dict[str, Any]:
    """Run what-if scenario on deep-cloned engine without affecting live simulation."""
...
def ai_status() -> dict[str, Any]:
    ...
    return {
        ...
        "vlm_acts": os.environ.get("TWIN_VLM_ACTS", "0") == "1"
    }
