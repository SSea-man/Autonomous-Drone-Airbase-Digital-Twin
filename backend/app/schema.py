"""
WareTwin Digital Twin State Specification (Pydantic v2)

Mirrors twin_state.js specifications: pure serializable state in meters, 100ms ticks.
All API and WebSocket responses are validated against these models.

Usage:
    state = TwinState.model_validate(json_dict)
    state.model_dump_json()            # WebSocket broadcast
    copy = state.model_copy(deep=True) # What-if simulation clone
"""
...
class _Base(BaseModel):
    """Base configuration: forbid extra fields, serialize enums as strings."""
...
class PerceivedObstacle(_Base):
    """Relative obstacle position detected by virtual LiDAR."""
    kind: Literal["ROBOT", "HUMAN", "RACK"]
    id: Optional[str] = None
    distance_m: float = Field(ge=0)
    bearing_deg: float          # Relative bearing (-180 to +180 deg)


class Perception(_Base):
    """Robot sensor perception field (270 deg / 4m) and obstacle avoidance state."""
    state: Literal["CLEAR", "SLOWING", "STOPPED", "OFF"] = "OFF"
    ahead_m: float = 4.0        # Forward clearance distance (m)
...
class RobotState(_Base):
    id: RobotId
    model: str = "AMR-L"
    floor: int = 1                       # Current floor level (Y elevation added in renderer)
    lift_id: Optional[str] = None        # Active elevator ID if in transit
...
class LiftState(_Base):
    """Elevator state: authoritative backend model for doors, platform height, and queue."""
    id: str
    state: LiftFsmState = "IDLE"
    floor: Optional[int] = 1             # Null during vertical transit
    target_floor: Optional[int] = None
    y: float = 0                         # Platform height (m) interpolated via smoothstep
    ...
    until_tick: int = 0
    fault: bool = False
    fault_remaining: int = 0             # Remaining ticks frozen during fault
    trips: int = 0
    busy_ticks: int = 0
...
class LiftKpi(_Base):
    """Elevator operational performance metrics."""
    trips: int = 0
    utilization: float = 0
    avg_wait_s: float = 0
    faults: int = 0
...
class HeatmapLayer(_Base):
    ...
    floor: int = 1                # Associated floor level
    cols: int
    rows: int
    values: list[float]
    window_ticks: int
...
class TaskBurst(_Inj):
    ...
    count: int = Field(gt=0, le=30)                  # Batch size limit (up to 30)
    priority: TaskPriority = TaskPriority.NORMAL
...
class MsgPatch(_Base):
    ...
    base_tick: int
    tick: int
    patch: dict[str, Any]          # Incremental delta patch generated from diff
...
class MsgWhatIfResult(_Base):
    ...
    request_id: Optional[str] = None   # Associates reply with CmdWhatIfRun.request_id
    result: WhatIfResult
...
class MsgError(_Base):
    ...
    code: str
    message: str
    request_id: Optional[str] = None   # Associated request ID (COPILOT_ASK / WHATIF_RUN)
...
class CmdWhatIfRun(_Base):
    ...
    request: WhatIfRequest
    request_id: Optional[str] = Field(default=None, max_length=32)   # Corresponds to client request_id
...
class VlmObserveBody(_Base):
    """REST /api/vlm/observe: accepts JPEG data URL up to 400 KB base64 payload."""
    camera_id: CameraId
