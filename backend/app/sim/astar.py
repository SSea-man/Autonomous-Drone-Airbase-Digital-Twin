"""
Grid A* Pathfinding (8-directional with corner cutting prevention).
grid.cells: 0 navigable, 1 obstacle, 2 walkway (1.6x cost).
blocked: temporary obstacle coordinates; cost_map: per-cell additional traversal costs.
Returns list of path waypoints excluding start; None if unreachable.
"""
...
def astar(grid: NavGrid, start: Cell, goal: Cell, blocked: Optional[set[Cell]] = None,
    ...
    g[s_idx] = 0.0
    # (f, tie_breaker_seq, idx) ensures stable priority queue order
    seq = 0
