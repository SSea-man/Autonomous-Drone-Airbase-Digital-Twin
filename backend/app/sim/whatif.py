...
What-if Simulation Engine
  ...
  Baseline  --clone--> Run N ticks --> Baseline KPI
  LIVE state ─┤
  Scenario  --clone--> Inject fault --> Run N ticks --> Scenario KPI -> Comparison Delta

Both clones branch from identical PRNG seeds and tick states to isolate injected variance.
"""
...
def _window_kpi(eng: SimEngine, start_tick: int, start_completed: int, start_wait: float, start_energy: float, duration: int, counts: dict[str, int]) -> dict[str, Any]:
    ...
    return {
        ...
        "replans": counts.get("ROUTE_REPLANNED", 0),     # Total count across what-if run duration
...
def run_whatif(base_eng: SimEngine, scen_eng: SimEngine, request: dict[str, Any], start_tick: int) -> dict[str, Any]:
    """Execute what-if simulation run across baseline and scenario engine clones."""
    ...
    return {
        ...
        # Auxiliary comparison metrics for UI table
