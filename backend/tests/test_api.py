"""FastAPI and WebSocket protocol endpoint tests."""
import json
...
def test_health_reports_dead_sim_task():
    """Health check: returns 503 if simulation loop is stalled."""
    import time as _t
    with TestClient(app) as client:
...
def test_patch_base_tick_chains_across_multi_tick_rounds():
    """Verifies incremental patch base_tick aligns with previous broadcast."""
    with TestClient(app) as client:
...
def test_sim_loop_survives_db_failure(monkeypatch):
    """Database exceptions must not interrupt live simulation loop."""
    import time as _t
