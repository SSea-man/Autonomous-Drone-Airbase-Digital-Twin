"""
Generate warehouse_layout.json configuration.
Usage: python gen_layout.py > warehouse_layout.json
Coordinates: X 0..100 (length), Z 0..70 (width) in meters. Z=0 denotes loading dock side.
"""
import json

W, D = 100, 70
CELL = 1.0
F2_ELEV = 8.0            # Mezzanine deck elevation (m)
F2 = (8, 40, 54, 62)     # Mezzanine footprint (x0, z0, x1, z1)
...
layout = {
    ...
    "sensors": [],
    "locations": [],
    "obstacles": [   # Structural columns: perimeter walls and rack boundary columns
...
# ── Zones: 4 quadrants reserving central thoroughfare and conveyor corridor
zones = {
    "A": (4, 10, 46, 32, "#3b82f6"),
...
# ── Racks: 4 rows of double-sided shelving per zone, 10 bays per row, 4m aisles
...
for zid, (x0, z0, x1, z1, _) in zones.items():
    ...
    for r in range(n_rows):
        ...
        for b in range(10):
            ...
            # Pick/drop access locations for task source/destination waypoints
...
# ── Level 2: 3 rack rows x 8 bays, 4m aisles
m_n = 0
for r in range(3):
...
# ── Vertical Reciprocating Conveyor (VRC) lifts
layout["lifts"] = [
    {"id": "LIFT-1", "cell": [51, 44], "floors": [1, 2], "ride_ticks": 60},
    {"id": "LIFT-2", "cell": [51, 56], "floors": [1, 2], "ride_ticks": 60},
]

# ── Conveyors: dual transverse lines and perimeter transfer conveyors
layout["conveyors"] = [
...
# ── Conveyor terminal equipment: intake hoppers and receiving units
...
# ── Charging: 6 automated charge stations
for i in range(6):
    x = 22 + i * 2.5
    layout["charging_stations"].append({
        # Charger cabinet and entry waypoint coordinates
...
# ── CCTV Cameras: 3 units per zone oriented along aisle axes
layout["cameras"] = []
for zid, (x0, z0, x1, z1, _) in zones.items():
...
layout["cameras"] += [
    ...
    # Level 2 cameras: absolute elevation at 8m
...
# ── Vehicle Fleet Spawn: 20 AGVs distributed across parking and charging
for i in range(20):
    if i >= 16:   # R17-R20 spawned on Level 2 mezzanine
        x = 14 + (i - 16) * 6.0
        z = 47
