 ...
 * Online: query backend LLM with rule-based fallback; clickable entity references.
 * Offline: local guidance when disconnected from server.
 */
...
export function Copilot() {
    ...
    const ask = (question) => {
        ...
        if (!text)
            return;
        // Substitute task token with active task ID
        ...
        setQ("");
        if (source !== "online") {
            setMsgs((m) => m.map((x) => (x.request_id === request_id ? { role: "ai", text: "Copilot runs on the backend. Start `uvicorn app.main:app` (the LOCAL badge means the backend is unreachable)." } : x)));
            return;
        }
        wsSend({ type: "COPILOT_ASK", request_id, question: sent });
    };
    const renderText = (t) => t.split(/(\b(?:R\d{2}|A\d{4}|E\d+|CV\d{2}|Zone [A-D])\b)/g).map((part, i) => {
        if (/^R\d{2}$/.test(part))
            return <button key={i} className="cite" onClick={() => select(part)}>{part}</button>;
        if (/^A\d{4}$/.test(part))
            return <button key={i} className="cite" onClick={() => setModal("tasks")}>{part}</button>;
        if (/^E\d+$/.test(part))
            return <button key={i} className="cite" onClick={() => setModal("audit")}>{part}</button>;
        return <span key={i}>{part}</span>;
    });
    return (<div className="copilot">
      ...
      <div className="chat" ref={listRef}>
        ...
        {msgs.map((m, i) => (<div key={i} className={"msg " + m.role}>
            ...
            {m.role === "ai" && !m.pending && (<div className="msg-meta">
                {m.model && <span className="model">{m.model}</span>}
                {m.citations && m.citations.length > 0 && <span>· cites {m.citations.map((c, j) => <button key={j} className="cite" onClick={() => { if (c.robot_id)
                select(c.robot_id);
            else if (c.task_id)
                setModal("tasks");
            else
                setModal("audit"); }}>{c.robot_id ?? c.task_id ?? c.event_id}</button>)}</span>}
              </div>)}
          </div>))}
      </div>
