/**
 * AI Operations Drawer: explainable dispatch decisions and operational metrics
 * Decision audit: lists assignment rationale and candidate ranking.
 */
...
export function OpsDrawer() {
    ...
    return (<aside className="drawer wide" role="dialog" aria-label="AI Operations" ref={trap} tabIndex={-1}>
      ...
      <div className="drawer-b">
        ...
        {decisions.slice(0, 12).map((d) => {
            ...
            return (<div key={d.id} className="decision">
              <div className="dec-h">
              ...
              {d.candidates.filter((c) => c.robot_id !== d.selected_robot).length > 0 && (<div className="rejected">Rejected: {d.candidates.filter((c) => c.robot_id !== d.selected_robot).map((c) => `${c.robot_id} — ${c.rejected_reason} (${c.score.toFixed(2)})`).join(" · ")}</div>)}
