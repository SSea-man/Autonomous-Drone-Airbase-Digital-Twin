/**
 * Fault Injection Drawer: Robot, Conveyor, Camera, Human Intrusion, Congestion, Task Burst
 * Active fault indicators derived from twin state with one-click clearance.
 */
...
export function ScenariosDrawer() {
    ...
    if (!open)
        return null;
    const fire = (inj) => simControl.inject(inj);
    const robots = Object.values(twin.robots);
    // Active injections derived from twin state
    const active = [
        ...robots.filter((r) => r.status === "OFFLINE").map((r) => ({ kind: "ROBOT_FAILURE", target: r.id, label: `${r.id} offline` })),
        ...Object.values(twin.conveyors).filter((c) => c.status === "ERROR").map((c) => ({ kind: "CONVEYOR_FAILURE", target: c.id, label: `${c.id} error` })),
        ...Object.values(twin.cameras).filter((c) => c.status === "OFFLINE").map((c) => ({ kind: "CAMERA_OFFLINE", target: c.id, label: `${c.id} offline` })),
        ...[...new Set(Object.values(twin.people).map((p) => p.zone).filter(Boolean))].map((z) => ({ kind: "HUMAN_INTRUSION", target: z, label: `Human in Zone ${z}` })),
        ...Object.values(twin.alerts).filter((a) => a.id.startsWith("traffic-")).map((a) => ({ kind: "TRAFFIC_CONGESTION", target: a.zone_id, label: `Traffic Zone ${a.zone_id}` })),
        ...Object.values(twin.lifts ?? {}).filter((l) => l.fault).map((l) => ({ kind: "LIFT_FAULT", target: l.id, label: `${l.id} fault` })),
    ];
    return (<aside className="drawer" role="dialog" aria-label="Scenario Injection" ref={trap} tabIndex={-1}>
      ...
      <div className="drawer-b">
        ...
        {active.map((a) => (<div key={a.kind + a.target} className="active-row">
...
function Row({ title, demo, desc, children }) {
    return (<div className="inj-row">
