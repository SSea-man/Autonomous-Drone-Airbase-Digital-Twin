/**
 * What-if Simulation: Scenario comparison and impact forecasting
 *  Evaluates scenario impact against baseline with key event detection and recommendations.
 *  Option to apply evaluated injection parameters to live twin.
 */
...
export function WhatIfDrawer() {
    ...
    const result = useStore((s) => s.whatif);
    const setResult = useStore((s) => s.setWhatIf);
    const [sel, setSel] = useState(new Set(["r07"]));
    const [dur, setDur] = useState(300);
    const [baseline, setBaseline] = useState(true);
    const [running, setRunning] = useState(false);
    const [err, setErr] = useState(null);
    useEffect(() => onWhatIfResult((r) => { pendingId.current = null; setResult(r); setRunning(false); setErr(null); }), [setResult]);
    const seq = useRef(0);
    const pendingId = useRef(null);
    useEffect(() => onWhatIfError((m, id) => { if (id === pendingId.current) {
    ...
    useEffect(() => { if (!open)
        return; const h = (e) => e.key === "Escape" && setDrawer(null); window.addEventListener("keydown", h); return () => window.removeEventListener("keydown", h); }, [open, setDrawer]);
    if (!open)
        return null;
    const injections = () => PRESETS.filter((p) => sel.has(p.id)).map((p) => p.build());
    const run = () => {
        if (source !== "online" || sel.size === 0)
            return;
        const request_id = `w${++seq.current}-${Date.now().toString(36)}`;
        pendingId.current = request_id;
        setRunning(true);
        setErr(null);
        markWhatIfPending(request_id);
    ...
    const fmt = (k, v) => k === "on_time_rate" || k === "utilization" ? `${Math.round(v * 100)}%` : k === "congestion_index" ? `${Math.round(v * 100)}%` : Number.isInteger(v) ? String(v) : v.toFixed(1);
    const pctText = (k, b, s) => (k === "on_time_rate" || k === "utilization" || k === "congestion_index") ? `${((s - b) * 100).toFixed(0) === "0" ? "±0" : ((s - b) * 100 > 0 ? "+" : "") + ((s - b) * 100).toFixed(0)} pt` : b ? `${s - b > 0 ? "+" : ""}${(((s - b) / b) * 100).toFixed(0)}%` : "";
    return (<aside className="drawer wide" role="dialog" aria-label="What-if Simulation" ref={trap} tabIndex={-1}>
      ...
      <div className="drawer-b">
        ...
        <div className="wi-list">
          {PRESETS.map((p) => (<label key={p.id} className={"wi-item" + (sel.has(p.id) ? " on" : "")}>
        ...
        {result && (<>
            ...
            <div className="wi-actions">
              <button className="btn danger" onClick={() => { for (const i of result.request.injections)
            simControl.inject(i); setDrawer("scenarios"); }}>Apply scenario to LIVE</button>
