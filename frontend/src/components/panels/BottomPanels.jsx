...
export function TaskQueuePanel() {
    ...
    return (<Panel title="Task Queue" action={<button className="link" onClick={() => setModal("tasks")}>View All Tasks →</button>}>
      <div className="table-wrap"><table className="dt">
        ...
        <tbody>
          {rows.map((t) => (<tr key={t.id} className={t.assigned_robot && t.assigned_robot === selected ? "sel" : ""} onClick={() => t.assigned_robot && select(t.assigned_robot)}>
...
export function ThroughputPanel() {
    ...
    // X-axis: recent 120 sim-minutes; Y-axis autoscaled
    const n = 120;
    ...
    const sx = (tick) => px + ((tick - firstTick) / ((n - 1) * 600)) * (W - px - 8);
    const sy = (v) => py + (1 - v / maxY) * (H - py - pb);
    ...
    return (<Panel title="Throughput" sub="(Today)">
      ...
      <div className="chart-wrap" style={{ marginTop: 4 }}>
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" preserveAspectRatio="none" style={{ display: "block" }}>
          ...
          {ySteps.map((v) => (<g key={v}><line x1={px} x2={W - 8} y1={sy(v)} y2={sy(v)} stroke="#1e293b" strokeWidth="1"/><text x={px - 6} y={sy(v) + 4} fill="#8b98ad" fontSize="9" textAnchor="end" fontFamily="JetBrains Mono, monospace">{v}</text></g>))}
...
export function RobotStatusPanel() {
    ...
    let off = 0;
    return (<Panel title="Robot Status">
      <div className="donut-wrap">
        <svg viewBox="0 0 130 130">
          <circle cx="65" cy="65" r={R} fill="none" stroke="#1e293b" strokeWidth={R - r}/>
          {parts.map((p) => {
            const len = (p.v / f.total) * C;
            const el = <circle key={p.k} cx="65" cy="65" r={R} fill="none" stroke={p.c} strokeWidth={R - r} strokeDasharray={`${len} ${C - len}`} strokeDashoffset={-off} transform="rotate(-90 65 65)"/>;
            off += len;
            return el;
        })}
