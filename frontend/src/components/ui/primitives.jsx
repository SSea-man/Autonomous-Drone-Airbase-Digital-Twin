export function Panel({ title, sub, action, children, grow, style }) {
    return (<section className={"panel" + (grow ? " grow" : "")} style={style}>
...
export function StatRow({ label, value, color, delta, deltaColor, big }) {
...
export function Dot({ color, size = 6 }) {
    return <span className="dot" style={{ background: color, width: size, height: size, borderRadius: "50%", display: "inline-block", flex: "none" }}/>;
}
export const Icon = {
