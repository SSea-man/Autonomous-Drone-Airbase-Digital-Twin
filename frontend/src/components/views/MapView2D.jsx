...
/** Top-down 2D map: navgrid obstacles, zones, conveyors, robots, and density overlays */
export function MapView2D({ mode }) {
    const allRobots = useStore((s) => s.twin.robots);
    const activeFloorSel = useStore((s) => s.activeFloor);
    const mapFloor = typeof activeFloorSel === "number" ? activeFloorSel : 1; // 2D map renders one floor at a time
    ...
    // Merge adjacent obstacle cells into rectangles via run-length encoding
    const blocks = useMemo(() => {
        const fp = mapFloor === 1 ? null : layout.floors.find((f) => f.id === mapFloor)?.footprint;
        const inFp = (c, r) => !fp || (c >= Math.min(...fp.map((p) => p[0])) && c < Math.max(...fp.map((p) => p[0])) && r >= Math.min(...fp.map((p) => p[1])) && r < Math.max(...fp.map((p) => p[1])));
        const out = [];
        for (let r = 0; r < grid.rows; r++) {
            let c = 0;
            while (c < grid.cols) {
                if (grid.cells[r * grid.cols + c] === 1 && inFp(c, r)) {
                    let e = c;
                    while (e < grid.cols && grid.cells[r * grid.cols + e] === 1 && inFp(e, r))
                        e++;
                    out.push([c, r, e - c]);
                    c = e;
                }
    ...
    // Real-time traffic density with Gaussian kernel diffusion
    // Cumulative traffic heatmap tracking aisle utilization over time
    ...
    const heat = useMemo(() => {
        ...
        if (layer) {
            // Downsampled to 2m grid and normalized
            for (let i = 0; i < Math.min(v.length, layer.values.length); i++)
                v[i] = layer.values[i] * 100;
        }
            ...
            if (src)
                for (let r = 0; r < g.rows; r++)
                    for (let c = 0; c < g.cols; c++) {
                        const t = src[r * g.cols + c];
                        if (t > 0)
                            v[Math.floor(r / cs) * cols + Math.floor(c / cs)] += t;
                    }
        }
        if (mode === "TRAFFIC") {
            // Density kernel radius ~5m with higher weighting for congested vehicles
            for (const r of Object.values(robots)) {
                if (r.fsm === "IDLE" || r.fsm === "CHARGING" || r.fsm === "OFFLINE")
                    continue;
                const w = r.velocity < 0.1 ? 60 : 30, cx = r.position[0] / cs, cz = r.position[2] / cs;
                for (let dr = -3; dr <= 3; dr++)
                    for (let dc = -3; dc <= 3; dc++) {
                        const rr = Math.floor(cz) + dr, cc = Math.floor(cx) + dc;
                        if (rr < 0 || cc < 0 || rr >= rows || cc >= cols)
                            continue;
                        v[rr * cols + cc] += w * Math.exp(-(dr * dr + dc * dc) / 3);
                    }
            }
        }
        ...
        // Recalculate on floor switch to prevent residual data from previous floor
    ...
    const heatColor = (t) => {
        ...
        let i = 1;
        while (i < stops.length - 1 && stops[i][0] < t)
            i++;
    ...
    return (<svg className="map2d" viewBox={`-2 -7 ${W + 4} ${D + 10}`} preserveAspectRatio="xMidYMid meet">
      ...
      {heat && (<g opacity="0.75">
          {Array.from(heat.v).map((s, i) => { const t = s / heat.max; if (t < 0.08)
            return null; return <rect key={i} x={(i % heat.cols) * heat.cs} y={Math.floor(i / heat.cols) * heat.cs} width={heat.cs} height={heat.cs} fill={heatColor(t)} opacity={Math.min(0.85, t + 0.15)}/>; })}
        </g>)}
      ...
      {layout.zones.filter((z) => (z.floor ?? 1) === mapFloor).map((z) => {
            const st = zones[z.id]?.status;
            const col = st === "BLOCKED" ? "#ef4444" : st === "CONGESTED" ? "#f97316" : z.color;
      ...
      {mapFloor !== 1 && layout.floors.filter((f) => f.id === mapFloor && f.footprint).map((f) => (<polygon key={f.id} points={f.footprint.map((p) => p.join(",")).join(" ")} fill="#14b8a6" fillOpacity="0.03" stroke="#14b8a6" strokeWidth="0.3" strokeDasharray="1.5 0.8"/>))}
      {layout.lifts.map((l) => (<g key={l.id}><rect x={l.cell[0] - 0.7} y={l.cell[1] - 0.7} width="2.4" height="2.4" fill="none" stroke="#a78bfa" strokeWidth="0.35"/><text x={l.cell[0] + 2} y={l.cell[1] + 0.6} fill="#a78bfa" fontSize="1.8">{l.id}</text></g>))}
      ...
      {Object.values(robots).map((r) => r.path.length > r.path_index && (<polyline key={"p" + r.id} points={[[r.position[0], r.position[2]], ...r.path.slice(r.path_index).map((c) => [c[0] + 0.5, c[1] + 0.5])].map((p) => p.join(",")).join(" ")} fill="none" stroke={r.id === selected ? "#fff" : "#22d3ee"} strokeWidth={r.id === selected ? 0.5 : 0.25} strokeOpacity={r.id === selected ? 1 : 0.5} strokeDasharray="1 0.6"/>))}
      {Object.values(robots).map((r) => {
            const sel = r.id === selected;
            return (<g key={r.id} transform={`translate(${r.position[0]},${r.position[2]})`} onClick={() => select(r.id)} style={{ cursor: "pointer" }}>
      ...
      {heat && (<g transform={`translate(${W - 22}, ${D - 3})`}>
