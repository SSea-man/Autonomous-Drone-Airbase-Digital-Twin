...
describe("A*", () => {
    ...
    it("finds a path between two shelf access points and never crosses obstacles", () => {
        ...
        for (const [c, r] of p)
            expect(grid.cells[r * grid.cols + c]).not.toBe(1);
        // Consecutive waypoints must be adjacent
    ...
    it("every location access point is walkable", () => {
        const grids = { 1: grid, 2: buildNavGrid(layout, 2) };
        for (const l of layout.locations) {
            const g = grids[l.floor ?? 1];
            const [c, r] = toCell(l.access_point[0], l.access_point[1]);
            expect(g.cells[r * g.cols + c], l.id).not.toBe(1);
        }
    });
    it("returns null when goal is walled in", () => {
        const g = { cols: 5, rows: 5, cells: new Uint8Array(25) };
        for (const [c, r] of [[1, 1], [2, 1], [3, 1], [1, 2], [3, 2], [1, 3], [2, 3], [3, 3]])
            g.cells[r * 5 + c] = 1;
...
describe("SimEngine", () => {
            ...
            if (t % 10 === 0) {
                ...
                for (let i = 0; i < rs.length; i++)
                    for (let j = i + 1; j < rs.length; j++) {
                        if (rs[i].floor !== rs[j].floor || rs[i].lift_id || rs[j].lift_id)
                            continue;
                        minD = Math.min(minD, Math.hypot(rs[i].position[0] - rs[j].position[0], rs[i].position[2] - rs[j].position[2]));
                    }
            }
        }
        expect(eng.state.kpi.operation.completed_today).toBeGreaterThan(40);
        expect(minD).toBeGreaterThanOrEqual(0.5);
    }, 30000); // 12000 ticks under parallel test load
    it("does not gridlock under compound failure (Demo 10)", () => {
        const eng = new SimEngine(layout, { seed: 42 });
        for (let t = 0; t < 3000; t++)
            eng.step();
        eng.inject({ kind: "CONVEYOR_FAILURE", conveyor_id: "CV03" });
        eng.inject({ kind: "HUMAN_INTRUSION", zone_id: "B", duration_ticks: 2000 });
    ...
    it("is deterministic for the same seed", () => {
        ...
        const c = new SimEngine(layout, { seed: 4 });
        for (let t = 0; t < 3000; t++)
            c.step();
    ...
    it("low battery triggers charging and task transfer", () => {
        ...
        for (let t = 0; t < 4000; t++) {
            eng.step();
            const r = eng.state.robots[busy.id];
            if (r.fsm === "TASK_TRANSFER")
                transferred = true;
            if (r.fsm === "CHARGING")
                charging = true;
            if (t < 50 && eng.state.recent_events.some((e) => e.type === "ROBOT_BATTERY_LOW" && e.robot_id === busy.id))
                lowEvent = true;
        }
...
describe("task rules", () => {
    it("rejects invalid task locations and survives a bad destination in state", () => {
        const eng = new SimEngine(layoutJson, { seed: 1 });
...
describe("multi-floor", () => {
    ...
    it("floor-2 grid only covers the mezzanine footprint", () => {
        const g2 = buildNavGrid(layout, 2);
        const at = (x, z) => g2.cells[Math.floor(z) * g2.cols + Math.floor(x)];
        expect(at(51.5, 44.5)).toBe(1); // Elevator shaft = obstacle
        expect(at(20.5, 47.5)).not.toBe(1); // Mezzanine walkway
        expect(at(80, 20)).toBe(1); // Beyond mezzanine footprint
    });
});
describe("lift state machine (spec §9–§14)", () => {
    ...
    it("lift never moves with an open gate; floor flips only after fully exiting the cabin", () => {
        ...
        const floorAt = {};
        const stageAt = {};
        const cabins = eng.state ? eng.layout.lifts.map((l) => [l.cell[0] + 0.5, l.cell[1] + 0.5]) : [];
        for (let i = 0; i < 24000 && t.status !== "COMPLETED"; i++) {
            eng.step();
            for (const L of Object.values(eng.state.lifts)) {
                if (L.state === "MOVING_UP" || L.state === "MOVING_DOWN") {
                    expect(L.door_f1).toBe("CLOSED");
                    expect(L.door_f2).toBe("CLOSED");
                    expect(L.floor).toBeNull(); // Elevator in motion belongs to no floor
                }
            }
            for (const r of Object.values(eng.state.robots)) {
                const prev = floorAt[r.id] ?? r.floor;
                if (r.floor !== prev) {
                    // Floor changes only after alighting is complete and cabin is cleared
    ...
    it("clearing a lift fault resumes the platform from the frozen height — no teleport", () => {
        const { eng, t } = boot();
        // Wait until robot boards elevator and motion starts
        let lid = null;
        for (let i = 0; i < 40000 && !lid; i++) {
            eng.step();
            for (const L of Object.values(eng.state.lifts))
                if (L.occupant && (L.state === "MOVING_UP" || L.state === "MOVING_DOWN"))
                    lid = L.id;
        }
        expect(lid).toBeTruthy();
        const L = eng.state.lifts[lid];
        // Inject fault mid-transit
        for (let i = 0; i < 20 && (L.state === "MOVING_UP" || L.state === "MOVING_DOWN") && Math.abs(L.y) < 4; i++)
            eng.step();
        ...
        for (let i = 0; i < 200; i++) {
            eng.step();
            expect(L.y).toBe(frozenY);
        } // Height remains frozen during fault
        eng.clearInjection("LIFT_FAULT", lid);
        let prevY = L.y;
        for (let i = 0; i < 2000; i++) {
            eng.step();
            expect(Math.abs(L.y - prevY), "platform teleported after fault clear").toBeLessThan(0.5); // Smooth motion without teleportation
            prevY = L.y;
        }
        for (let i = 0; i < 30000 && t.status !== "COMPLETED"; i++)
            eng.step();
        expect(t.status).toBe("COMPLETED");
    }, 120000);
    it("only one robot occupies a lift; boarding order is FIFO", () => {
        const eng = new SimEngine(layout, { seed: 11 });
        for (const dst of ["PACK-01", "SORT-01", "PACK-02"])
            eng.createTask({ type: "PICK", priority: "CRITICAL", source: dst === "PACK-01" ? "SHELF-M01" : dst === "SORT-01" ? "SHELF-M10" : "SHELF-M20", destination: dst });
        const boarded = [];
        const enq = {};
        for (let i = 0; i < 30000; i++) {
            ...
            for (const L of Object.values(eng.state.lifts)) {
                for (const f of ["1", "2"])
                    for (const rid of L.queue[f])
                        if (!(rid in enq))
                            enq[rid] = i;
                if (L.occupant && boarded[boarded.length - 1] !== L.occupant)
                    if (!boarded.includes(L.occupant))
                        boarded.push(L.occupant);
            }
            const riders = Object.values(eng.state.robots).filter((r) => r.lift_id);
            const byLift = new Map();
            for (const r of riders)
                byLift.set(r.lift_id, (byLift.get(r.lift_id) ?? 0) + 1);
            for (const n of byLift.values())
                expect(n).toBe(1);
        }
    ...
    it("lift fault reroutes waiting robots to the other lift; rider is never teleported", () => {
        const { eng, t } = boot();
        // Wait for robot to enter elevator routine
        let rid = null;
        for (let i = 0; i < 20000 && !rid; i++) {
            eng.step();
            rid = t.assigned_robot && eng.state.robots[t.assigned_robot].lift_stage ? t.assigned_robot : null;
        }
        ...
        for (let i = 0; i < 200; i++)
            eng.step();
        if (!r.lift_id) {
            // Unboarded robots reroute to alternate elevator
...
describe("State rehydration - local engine fallback on disconnect", () => {
    const snapshotOf = (e) => JSON.parse(JSON.stringify(e.state));
    it("moving robots do not teleport on state rehydration", () => {
        const a = new SimEngine(layout, { seed: 21 });
        let rid = null;
        for (let i = 0; i < 20000 && !rid; i++) {
            a.step();
            for (const r of Object.values(a.state.robots))
                if (r.fsm === "NAVIGATING" && !r.lift_stage && r.path.length - r.path_index > 6) {
                    rid = r.id;
                    break;
                }
        }
        ...
        expect(r1.fsm).not.toBe("PICKING"); // Maintain active navigation state
        expect(r1.path.length).toBeGreaterThan(0); // Path remains active
    }, 60000);
    it("maintains consistent task and event sequencing without ID collision", () => {
        const a = new SimEngine(layout, { seed: 22 });
        for (let i = 0; i < 3000; i++)
            a.step();
        const snap = snapshotOf(a);
        const b = new SimEngine(layout, { seed: 22, initialState: snap });
        const existing = new Set(Object.keys(snap.tasks));
        const t = b.createTask({ type: "PICK", priority: "NORMAL", source: "SHELF-A01", destination: "PACK-01" });
        expect(existing.has(t.id)).toBe(false);
        const maxE = Math.max(...snap.recent_events.map((e) => parseInt(e.id.slice(1), 10)));
        expect(parseInt(b.state.recent_events[0].id.slice(1), 10)).toBeGreaterThan(maxE);
    }, 60000);
    it("handles mid-elevator transition correctly on engine rehydration", () => {
        ...
        let riding = false;
        for (let i = 0; i < 30000 && !riding; i++) {
            a.step();
            riding = Object.values(a.state.robots).some((r) => r.lift_stage === "RIDING");
        }
        ...
        for (const [id, r] of Object.entries(b.state.robots)) {
            prevFloor[id] = r.floor;
            prevStage[id] = r.lift_stage;
        }
        for (let i = 0; i < 60000 && t2.status !== "COMPLETED"; i++) {
            b.step();
            for (const [id, r] of Object.entries(b.state.robots)) {
                if (r.floor !== prevFloor[id])
                    expect(prevStage[id], `${id} changed floor outside lift flow after handover`).toBe("ALIGHTING");
                prevFloor[id] = r.floor;
                prevStage[id] = r.lift_stage;
            }
        }
...
describe("Dispatch audit to actual lift binding", () => {
    it("matches dispatch audit record with assigned elevator", () => {
        ...
        for (let j = 0; j < 5000 && !actual; j++) {
            e.step();
            for (const [lid, L] of Object.entries(e.state.lifts))
                if (L.queue["1"].includes(rid) || L.queue["2"].includes(rid) || L.reserved_by === rid || L.occupant === rid)
                    actual = lid;
        }
...
describe("Rehydration edge cases", () => {
    const snapshotOf = (e) => JSON.parse(JSON.stringify(e.state));
    it("resumes transporting during obstacle avoidance rehydration without duplicate tasks", () => {
        const a = new SimEngine(layout, { seed: 31 });
        let rid = null;
        for (let i = 0; i < 30000 && !rid; i++) {
            a.step();
            for (const r of Object.values(a.state.robots))
                if (r.fsm === "TRANSPORTING" && r.load.current > 0 && !r.lift_stage && r.path.length - r.path_index > 5) {
                    rid = r.id;
                    break;
                }
        }
        expect(rid).toBeTruthy();
        // Congestion injection switches active robots to OBSTACLE_DETECTED
        a.inject({ kind: "TRAFFIC_CONGESTION", zone_id: "A", level: 0.8, duration_ticks: 300 });
        a.step();
        const snapFsm = a.state.robots[rid].fsm;
        expect(["OBSTACLE_DETECTED", "REPLANNING"]).toContain(snapFsm);
        const snap = snapshotOf(a);
        const tid = snap.robots[rid].current_task_id;
        const maxE = Math.max(...snap.recent_events.map((e) => parseInt(e.id.slice(1), 10)));
        const b = new SimEngine(layout, { seed: 31, initialState: snap });
        b.step();
        // Rehydration restores phase correctly without resetting to initial pickup
        expect(["TRANSPORTING", "REPLANNING"]).toContain(b.state.robots[rid].fsm);
        for (let i = 0; i < 60000 && b.state.tasks[tid] && !["COMPLETED", "TRANSFERRED", "FAILED"].includes(b.state.tasks[tid].status); i++)
            b.step();
        // Disallow duplicate TASK_STARTED event after state rehydration
        const dup = b.state.recent_events.filter((e) => parseInt(e.id.slice(1), 10) > maxE && e.type === "TASK_STARTED" && e.task_id === tid);
        expect(dup).toHaveLength(0);
    }, 120000);
    it("accounts for station slowdown during conveyor fault handover", () => {
        const a = new SimEngine(layout, { seed: 32 });
        const cv = layout.conveyors.find((c) => c.feeds);
        a.inject({ kind: "CONVEYOR_FAILURE", conveyor_id: cv.id });
        a.step();
        ...
        for (let i = 0; i < 90000 && !rid; i++) {
            ...
            for (const r of Object.values(a.state.robots)) {
                ...
                if (r.fsm === "DELIVERING" && prevFsm[r.id] !== "DELIVERING" && t && t.destination === cv.feeds) {
                    rid = r.id;
                    break;
                }
                prevFsm[r.id] = r.fsm;
        ...
        expect(rid).toBeTruthy(); // Active in DELIVERING state
        const b = new SimEngine(layout, { seed: 32, initialState: snapshotOf(a) });
        let n = 0;
        while (b.state.robots[rid].fsm === "DELIVERING" && n < 300) {
            b.step();
            n++;
        }
        expect(n).toBeGreaterThan(30); // Duration respects slowdown factor
        expect(n).toBeLessThanOrEqual(121);
    }, 120000);
    it("preserves avg_task_time_s across engine rehydration", () => {
        const a = new SimEngine(layout, { seed: 33 });
        for (let i = 0; i < 60000 && a.state.kpi.operation.completed_today < 5; i++)
            a.step();
        while (a.state.sim.tick % 10 !== 0)
            a.step(); // Advance KPI update
        const snap = snapshotOf(a);
        const avg0 = snap.kpi.operation.avg_task_time_s;
        ...
        for (let i = 0; i < 10; i++)
            b.step(); // Advance past KPI update interval
        if (b.state.kpi.operation.completed_today === done0) {
            expect(b.state.kpi.operation.avg_task_time_s).toBe(avg0); // Preserves metric
        }
        else {
            expect(b.state.kpi.operation.avg_task_time_s).toBeGreaterThan(0); // Metric remains positive
        }
    }, 120000);
});
describe("Lift shaft as navigation obstacle", () => {
    it("blocks elevator shafts on both floor grids while maintaining navigable queue slots and exits", () => {
        for (const fl of [1, 2]) {
            const g = buildNavGrid(layout, fl);
            for (const l of layout.lifts) {
                expect(g.cells[l.cell[1] * g.cols + l.cell[0]], `${l.id} cabin F${fl}`).toBe(1);
                // Shaft depth envelope safety clearance
                for (const dc of [-1, 0, 1])
                    for (const dr of [-2, 2])
                        expect(g.cells[(l.cell[1] + dr) * g.cols + (l.cell[0] + dc)], `${l.id} face(${dc},${dr}) F${fl}`).toBe(1);
                for (let i = 0; i < 3; i++)
                    expect(g.cells[l.cell[1] * g.cols + (l.cell[0] - 4 - i)], `${l.id} queue${i} F${fl}`).not.toBe(1);
                expect(g.cells[l.cell[1] * g.cols + (l.cell[0] - 2)], `${l.id} gate cell F${fl}`).not.toBe(1); // Gate access waypoint
    ...
    it("ensures A* paths never intersect elevator shafts", () => {
        // Mezzanine support columns blocked on Level 1
        const cols = layout.columns ?? [];
        if (!cols.length)
            throw new Error("layout.columns missing");
        const gc1 = buildNavGrid(layout, 1);
        ...
        const inColumn = (x, z) => cols.some(([cx, cz]) => Math.abs(x - cx) < 0.45 && Math.abs(z - cz) < 0.45);
        ...
        const inShaft = (x, z) => layout.lifts.some((l) => Math.abs(x - (l.cell[0] + 0.5)) < 1.4 && Math.abs(z - (l.cell[1] + 0.5)) < 1.4);
        // Bounding box clearance check around elevator shaft
        const bodyInShaft = (x, z, h) => {
            ...
            for (const [ex, ez] of [[SIM.ROBOT_HALF_LEN, SIM.ROBOT_HALF_W], [SIM.ROBOT_HALF_LEN, -SIM.ROBOT_HALF_W], [-SIM.ROBOT_HALF_LEN, SIM.ROBOT_HALF_W], [-SIM.ROBOT_HALF_LEN, -SIM.ROBOT_HALF_W]]) {
                ...
                if (layout.lifts.some((l) => Math.abs(px - (l.cell[0] + 0.5)) < 1.4 - 0.02 && Math.abs(pz - (l.cell[1] + 0.5)) < 1.8 - 0.02))
                    return true;
            }
            return false;
        };
        for (let i = 0; i < 20000; i++) {
            ...
            for (const r of Object.values(eng.state.robots)) {
                for (let pi = r.path_index; pi < r.path.length; pi++) {
                    if (inShaft(r.path[pi][0] + 0.5, r.path[pi][1] + 0.5))
                        throw new Error(`${r.id} path crosses shaft at tick ${eng.state.sim.tick}`);
                }
                if (!r.lift_stage && !r.lift_id && inShaft(r.position[0], r.position[2]))
                    throw new Error(`${r.id} inside shaft outside lift flow at tick ${eng.state.sim.tick}`);
                const inFlow = r.lift_id || r.lift_stage === "BOARDING" || r.lift_stage === "RIDING" || r.lift_stage === "ALIGHTING";
                if (!inFlow && bodyInShaft(r.position[0], r.position[2], r.heading))
                    throw new Error(`${r.id} body clips shaft structure at tick ${eng.state.sim.tick} (${r.position[0].toFixed(2)},${r.position[2].toFixed(2)})`);
                if (r.floor === 1 && !r.lift_id) {
                    if (inColumn(r.position[0], r.position[2]))
                        throw new Error(`${r.id} inside a support column at tick ${eng.state.sim.tick}`);
                    for (let pi = r.path_index; pi < r.path.length; pi++)
                        if (inColumn(r.path[pi][0] + 0.5, r.path[pi][1] + 0.5))
                            throw new Error(`${r.id} path crosses a support column`);
                }
            }
        }
    }, 180000);
});
describe("Elevator exit clearance and oriented bounding box constraints", () => {
    it("ensures simulation door geometry matches 3D shaft dimensions", () => {
        expect(SIM.LIFT_SHAFT_HALF_X * 2).toBe(LIFT_SHAFT.W);
        expect(SIM.LIFT_DOOR_HALF_W).toBe(LIFT_SHAFT.LEAF);
    });
    it("verifies 3-stage elevator alighting with OBB clearance and heading constraints", () => {
        const eng = new SimEngine(layout, { seed: 5 });
        const tasks = [
        ...
        const dirs = new Set();
        const liftsSeen = new Set();
        const loads = new Set();
        const prev = {};
        for (let i = 0; i < 90000; i++) {
            eng.step();
            for (const r of Object.values(eng.state.robots)) {
                if (r.lift_stage !== "ALIGHTING" || !r.lift_id) {
                    prev[r.id] = undefined;
                    continue;
                }
                ...
                // OBB corners of rotated chassis
                ...
                // Chassis within door portal aperture during threshold crossing
                ...
                for (const [a, b] of edges) {
                    ...
                    if ((ax - doorPlane) * (bx - doorPlane) < 0) {
                        ...
                        if (Math.abs(zc - cz) > SIM.LIFT_DOOR_HALF_W + 1e-6)
                            throw new Error(`${r.id} body sweeps into door frame at z=${zc.toFixed(2)} (x=${x.toFixed(2)}, θ=${h.toFixed(2)})`);
                    }
                }
                if (x > cx + 0.1)
                    throw new Error(`${r.id} moved east inside shaft`);
                ...
                if (pv) {
                    ...
                    while (dh < -Math.PI)
                        dh += 2 * Math.PI;
                    if (Math.abs(dh) > 0.09 && Math.hypot(x - pv.x, z - pv.z) > 0.02)
                        throw new Error(`${r.id} walks while turning (Δθ=${dh.toFixed(2)}, Δp=${Math.hypot(x - pv.x, z - pv.z).toFixed(3)})`);
                }
                prev[r.id] = { x, z, h };
                if (L.floor !== null)
                    dirs.add(`${r.floor}->${L.floor}`);
                liftsSeen.add(l.id);
                loads.add(r.load.current > 0);
                // OBB intersection prevention between alighting and queued robots
                for (const b of Object.values(eng.state.robots)) {
                    if (b.id === r.id || !["TO_LIFT", "QUEUED", "BOARDING"].includes(b.lift_stage ?? ""))
                        continue;
                    if (L.floor === null || b.floor !== L.floor)
                        continue;
            ...
            if (tasks.every((t) => t.status === "COMPLETED" || t.status === "TRANSFERRED" || t.status === "FAILED") && dirs.size >= 2)
                break;
        }
...
describe("Lift exit orientation toward destination", () => {
    it("selects the optimal exit closest to subsequent destination", () => {
        const eng = new SimEngine(layout, { seed: 1 });
        const pick = eng.pickLiftExit.bind(eng);
        for (const fl of [1, 2]) {
            for (const l of layout.lifts) {
...
describe("Structural pillars and charging dock alignment", () => {
    it("registers structural columns in layout obstacles without obstructing conveyors", () => {
        const obs = layout.obstacles ?? [];
        expect(obs.length).toBeGreaterThan(0);
        const g = buildNavGrid(layout, 1);
        for (const o of obs) {
            ...
            if (o.kind !== "PILLAR")
                continue; // Conveyor end equipment located at terminal points
    ...
    it("blocks grid around conveyor end equipment while preserving clearance in inner lanes", () => {
        const eq = (layout.obstacles ?? []).filter((o) => o.kind === "CONVEYOR_EQUIP");
        expect(eq.length).toBeGreaterThan(0);
        for (const cv of layout.conveyors) // Equipment footprint at conveyor endpoints
        ...
        const g = buildNavGrid(layout, 1);
        const at = (c, r) => g.cells[r * g.cols + c];
        for (let r = 34; r <= 60; r++) { // Perimeter conveyor wall clearance
            expect(at(0, r), `west wall strip row ${r}`).toBe(1);
            expect(at(1, r), `west wall strip row ${r}`).toBe(1);
            expect(at(98, r), `east wall strip row ${r}`).toBe(1);
            expect(at(99, r), `east wall strip row ${r}`).toBe(1);
        }
        for (let r = 37; r < 58; r++) { // Main navigable thoroughfare
            for (const c of [2, 3, 4, 5])
                expect(at(c, r), `west inner lane (${c},${r})`).not.toBe(1);
            for (const c of [93, 94, 95, 96, 97])
                expect(at(c, r), `east inner lane (${c},${r})`).not.toBe(1);
        }
        for (let r = 33; r < 37; r++) // Conveyor crossover thoroughfares
    ...
    it("verifies robots dock centered on charger pads without overlaps", () => {
        const e = new SimEngine(layout, { seed: 3 });
        for (const rid of Object.keys(e.state.robots).slice(0, 3))
            e.inject({ kind: "ROBOT_BATTERY_SET", robot_id: rid, battery: 25 });
        const pads = layout.charging_stations.map((c) => [c.position[0], c.position[2] - 1.9]);
        const seen = new Set();
        for (let i = 0; i < 20000; i++) {
            e.step();
            ...
            for (const r of chg) {
                ...
                if (d >= 0.05)
                    throw new Error(`${r.id} not docked on a pad (d=${d.toFixed(2)})`);
                if (Math.abs(r.heading - Math.PI / 2) >= 0.01)
                    throw new Error(`${r.id} not facing the charger`);
            }
            for (let a = 0; a < chg.length; a++)
                for (let b = a + 1; b < chg.length; b++)
            ...
            for (const r of chg)
                seen.add(r.id);
            if (chg.length >= 2) {
                expect(chg.length).toBeGreaterThanOrEqual(2);
                return;
            } // Concurrent charging validation
        }
