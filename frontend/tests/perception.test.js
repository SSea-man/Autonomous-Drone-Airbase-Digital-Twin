...
describe("perception", () => {
    it("keeps following robots >= PERC_STOP apart and reports obstacles", () => {
        const eng = new SimEngine(layoutJson, { seed: 42 });
        let minD = 9, stops = 0, slows = 0, seen = 0;
        for (let t = 0; t < 6000; t++) {
            eng.step();
            const rs = Object.values(eng.state.robots);
            for (const r of rs) {
                if (r.perception.state === "STOPPED")
                    stops++;
                if (r.perception.state === "SLOWING")
                    slows++;
                if (r.perception.obstacles.length)
                    seen++;
            }
            for (let i = 0; i < rs.length; i++)
                for (let j = i + 1; j < rs.length; j++) {
                    const a = rs[i], b = rs[j];
                    if (a.floor !== b.floor || a.lift_id || b.lift_id)
                        continue;
        ...
        expect(stops + slows).toBeGreaterThan(0);
